...
export async function executeVerifyPoliciesSortCapability(page: Page, row: MatrixRow): Promise<void> {
  if (!POLICIES_SUPPORTED_PERSONAS.has(row.persona)) {
    throw new Error(`Row ${row.caseNo}: Verify Policies Sort is not yet supported for persona "${row.persona}"`);
  }
