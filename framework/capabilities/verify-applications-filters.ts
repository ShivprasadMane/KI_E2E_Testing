...
export async function executeVerifyApplicationsFiltersCapability(page: Page, row: MatrixRow): Promise<void> {
  if (!APPLICATIONS_SUPPORTED_PERSONAS.has(row.persona)) {
    throw new Error(`Row ${row.caseNo}: Verify Applications Filters is not yet supported for persona "${row.persona}"`);
  }
