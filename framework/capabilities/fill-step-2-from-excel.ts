...
export async function executeFillStep2FromExcelCapability(page: Page, row: MatrixRow): Promise<void> {
  const data = primeFormDataContext(page, row);
  const wizard = new ApplicationCreatePage(page, row.persona);
  await wizard.waitForStep('investment-amount');
  await wizard.fillInvestmentAmount(data);
  await wizard.saveAndProceedInvestmentAmount(data.initialAmount);
