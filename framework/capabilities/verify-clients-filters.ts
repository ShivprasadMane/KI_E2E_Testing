...
export async function executeVerifyClientsFiltersCapability(page: Page, row: MatrixRow): Promise<void> {
  if (!CLIENTS_SUPPORTED_PERSONAS.has(row.persona)) {
    throw new Error(`Row ${row.caseNo}: Verify Clients Filters is not yet supported for persona "${row.persona}"`);
  }
