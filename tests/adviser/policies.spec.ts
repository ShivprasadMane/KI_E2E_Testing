...
test.describe('Financial Adviser policies tab @adviser @adviser-policies', () => {
  test.beforeEach(async ({ page }) => {
    const policies = new PoliciesPage(page, 'adviser');
    await policies.open();
  });
  ...
  test('view details opens policy detail sections', async ({ page }) => {
    ...
    const detail = new PolicyDetailLayoutPage(page, 'adviser');
